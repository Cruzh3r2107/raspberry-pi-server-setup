...
- **Sport Kiosk** (Port 3000): Live sports scores kiosk display
- **Actual Budget** (Port 5006): Personal finance and budget management
...
Raspberry Pi 5 (192.168.1.154)
    ...
    ├── Sport Kiosk → Redis → /mnt/storage/sport-kiosk
    ├── Actual Budget → /mnt/storage/actual-budget
...
├── sport-kiosk/               # Sport kiosk cache data
│   └── redis/                # Redis cache (TTL: 1-15 minutes)
├── actual-budget/             # Budget data and sync files
...
| Sport Kiosk | `~/home-server/sport-kiosk/docker-compose.yml` | Inline env vars |
| Actual Budget | `~/home-server/actual-budget/docker-compose.yml` | Inline env vars |
...
cd ~/home-server/sport-kiosk && docker compose up -d
...
cd ~/home-server/sport-kiosk && docker compose down
...
cd ~/home-server/sport-kiosk && docker compose restart
cd ~/home-server/actual-budget && docker compose restart
...
### Sport Kiosk Configuration

**Location:** `~/home-server/sport-kiosk/`
...
**Docker Compose Notes:**
- Three services: frontend (React/Nginx), backend (Node.js), redis (cache)
- Redis volume: `/mnt/storage/sport-kiosk/redis:/data`
- Restart policy: `unless-stopped`

**Performance:**
- Normal idle: Backend 5-10% CPU, 50-100MB RAM
- Frontend 1-3% CPU, 20-40MB RAM
- Redis 1-2% CPU, 10-20MB RAM
- Fetches ESPN API every 1 minute for live scores

**Current Status:**
- NBA fully functional with live scores and schedules
- Other sports (NFL, F1, Cricket, MLB, Tour de France) coming soon

### Actual Budget Configuration

**Location:** `~/home-server/actual-budget/`
...
- **`sport-kiosk/README.md`** - Sports kiosk display setup, API endpoints, tablet configuration
- **`actual-budget/README.md`** - Personal finance and budget management setup
